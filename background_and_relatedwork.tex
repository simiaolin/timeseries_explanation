...
\subsection{DBA}
As can bee seen from its name, DTW Barycenter Averaging (DBA) makes use of DTW. It first initializes the average as the medoid of the time series instances, which has the smallest sum of distances to all the other instances. After the initialization, DBA updates the average iteractively. At each iteraction, it first computes the DTW between every sequence and the old average. For each coordinate in the old average, in every sequence there are one or more coordinates that are mapped to it. finding the coordinates that are associated to averaging the warping of other time series instances from the current average iteractively. Note that the warping of the time series instance from the current average is calculated with DTW. 
