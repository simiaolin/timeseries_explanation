...
\begin{center}$must-link(A, B) \wedge must-link(B, C) \Rightarrow must-link(A, C)$\end{center} \label{cobra:entailment}

\noindent ,and cannot-link constraints have entailment property:

\begin{center}$must-link(A, B) \wedge cannot-link(B, C) \Rightarrow cannot-link(A, C)$\end{center} \label{cobra:transitivity}
...
\begin{algorithm}[h]
	\caption{Constraint-based Repeated Aggregation \cite{vancraenendonck2018cobra}}
	\label{algo:cobra}
	\begin{algorithmic}[1]
...
\begin{algorithm}[h]
	\caption{Constraint-based Repeated Aggregation and Splitting \cite{van2018cobras}}
	\label{algo:cobras}
