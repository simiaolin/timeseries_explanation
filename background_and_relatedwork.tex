...
Euclidean distance is commonly used in measuring distances between two points, two vectors, etc. It is simple and competitive for many senarios. However, it is not the best choice in some domains, such as time series. Time series data are sensitive to time axis. Even though they share similar shapes they might not match well in Euclidean distance because their shifts or speed might not sychronize. A new way of measuring distance between time series that can capture their similarities is thus proposed, which is DTW(Dynamic Time Wrapping)
\subsection{DTW}
Dynamic time warping(DTW) is a well-known method to measure the distance between time series instances. \cref{fig:dtw_0}, \cref{fig:dtw_1} and \cref{fig:dtw_2} together give an intuitive illustration of the difference between Euclidean distance and DTW. \cref{fig:dtw_0} shows two time series, A and B, respectively. We are interested in their similarity. \cref{fig:dtw_1} illustrates how points in A and B are mapped to each other in Euclidean distance. We can see that the point of index $ i $ in A is mapped to point of index $ i $ in B. Unlike Euclidean distance, \cref{fig:dtw_2} shows that in DTW, there is no such necessity that point of index $ i $ in A is mapped to point of index $ i $ in B. Actually in this case point of index $i  $ in A is mapped to point of index $ i+2 $ in B. DTW is designed in such way to find optimal alignment between two time series. More details about DTW algorithm is shown in \cref{algo:dtw}. Intuitively, it uses dynamic programming to map two time series in a way that their peaks and their bottoms are mapped together. In other words, DTW maps their trends instead of their time axis.
...
\begin{figure}[!htb]
	...
	\begin{minipage}[t]{0.32\textwidth}
		\centering
		\includegraphics[width=\linewidth]{pics/related_work/dtw_0}
...
\begin{minipage}[t]{0.32\textwidth}
	\centering
	\includegraphics[width=\linewidth]{pics/related_work/dtw_1}
...
\begin{minipage}[t]{0.32\textwidth}
	\centering
	\includegraphics[width=\linewidth]{pics/related_work/dtw_2}
	\caption{DTW(Dynamic time series wrapping)}
	\label{fig:dtw_2}
\end{minipage}
