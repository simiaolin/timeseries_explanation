\chapter{Explaining the relation between two given sequences}
...
As for two sequences in different clusters, we can slightly change the way we find the must-link chain above to explain why they are in two clusters. As long as the query budget is adequate, there is only one reason that two clustered are not merged - there exists at least one cannot-link between these two clusters. Combining this cannot-link with the must-links inside two clusters, we can get a constraint chain ($\mathit{must\mbox{-}link \, chain - cannot\mbox{-}link - must\mbox{-}link \, chain}$) that can explain why two sequences are separated into two clusters. We illustrate it together with an example in \cref{fig:transitivity}.
...
\FloatBarrier
\section{Experiments}
We design an interactive system that allows users to designate the sequence ids of two sequences in which they have an interest in their relation. Every time they input two indices, we plot the constraint chain for them. There is a must-link between sequences with the same color except when they are two sequences at the beginning or the end of the constraint chain. In that case, there might not exist a must-link between them. Instead, they are in the same super-instance and one of them is the representative of the super-instance. There is a cannot-link between sequences that are located on the border where the color changes. The datasets and configuration are the same as that of experiments in \cref{cha:properties_of_a_cluster}.
...
\FloatBarrier
We are also interested in why two sequences are not clustered together. For example, we want to explore why the sequence of id 84 and sequence of id 198 are in two clusters. We provide the ids to the system, and it shows us the result in \cref{fig:member_cl_ecg}. Here the sequences in red are in the same cluster, and sequences in green are in another cluster. The constraint link is $\mathit{84-63-77-6-131-125-198}$. There is a cannot-link between the pink sequence of id 63 and the purple sequence of id 77. The sequence of id 84 is linked to the sequence of id 63 directly, while the sequence of id 198 is linked to sequence 77 via must-links $\mathit{77-6-131-125-198}$. 
...
In \cref{fig:member_ml_trace}, we can see that the sequence of id 55 is in the same cluster with the sequence of id 115 because of the must-links $\mathit{115-173-55}$. 
...
\FloatBarrier
In \cref{fig:member_cl_trace}, we can see that seqeucne of id 11 and sequece of id 21 are in two different clusters because of the constraint chain $\mathit{11-3-172-38-9-190-146-21}$, where $\mathit{38-9}$ is a cannot-link.
...
\FloatBarrier
\section{Conclusion}
In this chapter, we propose a new way to solve the question in the last chapter why two sequences are in the same cluster, or why they are in different clusters. We solve in a way making good use of the obtained constraints in the process the $\mathrm{COBRAS^{TS}}$. Our system provides users the chance to look at the clustering result of $\mathrm{COBRAS^{TS}}$ in a more specific perspective. More importantly, we can convince users that their expert knowledge is truly and well obtained and exploited in $\mathrm{COBRAS^{TS}}$. We can also explain how their knowledge contributes to the result, in particular, in explaining the relation between two given time series instances.

Our newly proposed idea is quite a general one. It can not only be used to explain $\mathrm{COBRAS^{TS}}$, but can also be used to explain other variants of COBRAS as long as the instances are visually friendly. However, the idea is also relatively naive and there are a few things that can be researched to get a better explanation.

\begin{enumerate}
	\item In $\mathrm{COBRAS^{TS}}$, we obtain constraints not only in the merging stage but also in the stage that determines the splitting level in the splitting stage. These constraints are all recorded in the same constraint pool. In other words, not all the constraints in the constraint pool are used in the merging stage. However, only constraints that have been used to merge clusters should be considered when constructing the must-link chain mentioned above. For cannot-links there is no such problem because they just prohibit the merging and do not lead to a merging. A possible solution to this problem is to record the must-links that have been truly used in the merging stage separately. When finding the must-link chain between two sequences, we only consider the must-links in this new must-link pool.
	
	\item When finding the must-link chain between two sequences, we use BFS among the must-links. In this way, we can get the shortest must-link chain. However, are the must-links on the chain the latest obtained must-links? We would like to find the latest must-links because they are more likely to form the latest clusters. The earlier obtained must-links have a greater chance to be used in merging clusters and then abandoned in the later splitting stage. One possible solution is to record the "age" of the must-links. When finding the must-link chain between two sequences, we find the chain on which the must-links are young. 
	
	\item When the number of sequences in the dataset and the number of queries are large, the must-link chain we obtain might be too long and thus not user-friendly. One possible solution is to provide users a chance to choose the length of the must-link chain, and only return a must-link chain no longer than that length. To shorten the length of the must-link chain, we can sort the similarities between consecutive sequences on the chain first and only retain the most dissimilar ones, as those are in greater need of must-links to illustrate their relations.
	
	
	%\item Apart from merging stage, there is also splitting stage in $\mathrm{COBRAS^{TS}}$. Suppose we have super-instances $\mathit{S_1, S_2, S_3}$ in cluster $ \mathcal{C}_1 $, and there is a must-link between $\mathrm{S_2}$ and $\mathit{S_1}$, a must-link between $\mathrm{S_2}$ and $\mathrm{S_3}$. The sequences in $\mathrm{S_1}$ can be connected to sequences in $\mathrm{S_3}$ using the method we propose above. However, there exists the chance that $\mathrm{S_2}$ is removed from cluster $ \mathcal{C}_1 $ in the later splitting stage. If $\mathrm{S_2}$ is removed from cluster $ \mathcal{C}_1 $, the must-link between $\mathrm{S_2}$ and $\mathrm{S_1}$ and the must-link between $\mathrm{S_2}$ and $\mathrm{S_3}$ are not removed from the constraint pool, so when finding the constraint chain between sequences in $\mathrm{S_1}$ and $\mathrm{S_3}$ we might still find the 
\end{enumerate}
