...
Firstly, find which two clusters the two target sequences are grouped into, then find a cannot-link between two clusters. Specifically, find a cannot-link between two super-instances which belong to two clusters respectively. More specifically, find a cannot-link between two representatives of two super-instances which belong to two clusters respectively. There might exists more than one such cannot-link, we just randomly pick one. In this case, a cannot-link between $\mathit{I_3}$ and $\mathit{I_4}$ satisfies our requirement as $\mathit{I_3}$ is the representative of super-instance $\mathit{S_3}$ in cluster $ \mathcal{C}_1 $, while $\mathit{I_4}$ is the representative of super-instance $\mathit{S_4}$ in cluster  $ \mathcal{C}_2 $. 

Secondly, connect each sequence to the representative of the super-instance it belongs to. If the sequence itself is already the representative of the super-instance, skip this step. In this case, we connect $\mathit{a}$ to $\mathit{I_1}$, and connect $\mathit{c}$ to $\mathit{I_5}$.

Thirdly, for the representatives found in step one and step two that are in the same cluster, conduct BFS in the constraint pool and connect them with must-link chain. For instance, we find $\mathit{I_3}$ in $ \mathcal{C}_1 $ in step one and $\mathit{I_1}$ in $ \mathcal{C}_1 $ in step two, so we connect them using must-link chain $\mathit{I_1-I_2-I_3}$. Similarly, as we find $\mathit{I_4}$ in $ \mathcal{C}_2 $ in step one and $\mathit{I_5}$ in $ \mathcal{C}_2 $ in step two, we connect them using must-link chain $\mathit{I_4-I_5}$.
...
\section{Experiments}
We design an interactive system that allows users to designate the sequence ids of two sequences which they have interest in their relation. Every time they input two indices, we plot the constraint chain for them. Sequences with same color means there is a must-link between them except when they are two sequences at the begining or at the end of the constraint chain. In that case, there might not exist a must-link between them. Instead, they are in the same super-instance and one of them is the representative of the super-instance. There is a cannot-link between sequences which are located on the border where color changes. The datasets and configuration are the same as that of experiments in \cref{cha:properties_of_a_cluster}.
...
We are interested in why the blue sequence with a high peak with value around 3 at index around 92 could be clustered together with other sequences that do not have such a peak, so we provide the syetem the id of the blue sequence and the id of a sequence in the same cluster without such a peak, which are 125 and 66 respectively. The result is shown in \cref{fig:member_ml_ecg}. From bottom to top are the sequences in the computed constraint chain. The constrain link is $\mathit{66-78-125}$. We can now clearly knows that it is sequence of id 78 that links the sequence of id 125 with a high peak at the tail and the sequence of id 66 together into the same cluster.
...
We are also interested in why two sequences are not clustered together. For example, we want to explore why sequence of id 84 and sequence of id 198 are in two clusters. We provide the ids to the system, and it shows us the result in \cref{fig:member_cl_ecg}. Here the sequences in red are in a same cluster, and sequences in green are in another cluster. The constraint link is $\mathit{84-63-143-125-198}$. Thers is a cannot-link between red sequence of id 143 and green sequence of id 125. Sequence of id 84 is linked to sequence of id 143 via must-links $\mathit{84-63-143}$, while sequence of id 198 is linked to sequence 125 via must-links $\mathit{198-125-143}$. 
...
In \cref{fig:member_ml_trace}, we can see that sequece of id 55 is in the same cluster with sequence of id 115 because of the must-links $\mathit{55-173-115}$. 
