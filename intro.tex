...
\section{\textbf{Our contributions}}
In this thesis, we propose two new ways to explain the time series clustering. 

Our first idea is to provide an overview of the clustering result by showing the properties of time series instances inside one cluster. We mainly show their DBA and deviation. Considering the instances are time series instances, we propose new types of deviation that can capture the alignment information of the time series data. The idea is new and it helps users build insight into the fact that DTW sacrifices the similarities of time series instances on the X-axis (the horizontal, time axis) to win similarities of time series instances on the Y-axis (the vertical, value-axis).

We not only illustrate the clustering result of $ \mathrm{COBRAS^{TS}} $ from a general perspective but also from a specific perspective. Our second idea is to show the relationship chain between two specific instances with constraint information provided by users to explain why they have been clustered together or apart. Moreover, our proposed technique to chain two time series instances with constraints, is not designed specifically for time series data. It can also be applied in other variants of COBRAS as long as the instances are visualization-friendly. The idea of chaining two instances with the constraints can be also used in explaining other constraint-based semi-supervised learning.

We use Python to implement our proposed ideas. Our codes are publicly available on Github pages shown in \cref{sec:github}.
 
\textbf{Structure}. After providing background material in \cref{cha:related_work} and problem definition in \cref{cha:problem_definition}, We introduce in \cref{cha:properties_of_a_cluster} different types of deviation for time series instances in one cluster. We follow in \cref{cha:cluster_membership}  illustrating the relationship chain between two time series instances with the constraints obtained in the process of $ \mathrm{COBRAS^{TS}}$. We conclude this thesis in \cref{cha:conclusion} with our proposed techniques and possible improvement for future work.
%%% Local Variables: 
%%% mode: latex
%%% TeX-master: "thesis"
%%% End: 
